...
/**
 * Paralel perspektif seçimi. İçerik (ParallelPickList) ile dialog sarmalayıcı
 * (ParallelPickDialog) ayrıdır: DayEntrySheet kendi sheet'inin üzerinde
 * dialog'u kullanır; düzenleme modalı ise içeriği KENDİ dialog'unun içinde
 * görünüm olarak gösterir — iç içe/üst üste iki Radix dialog'u kırılgandı
 * (alttaki kendini kapatıp akış durumunu limboda bırakabiliyordu).
 *
 * Liste: her kategorinin TÜM alt kategori ağacı (iç içe olanlar yol etiketiyle
 * "Öğünler › Kahvaltı"); kategori adını taşıyan kesikli çip kategorinin
 * kendisini (gizli kök üzerinden, gerekirse o an yaratılarak) seçer.
 */
export function ParallelPickList({
  excludeCategoryId,
  hiddenSubIds,
  selected,
  onAdd,
  ...
  return (
    <div className="flex flex-col gap-5">
      {groups.map(({ category, subs, rootSub }) => {
        ...
        const rootHidden = !!rootSub && !!hiddenSubIds?.has(rootSub.id);
...
/** Dialog sarmalayıcı — DayEntrySheet gibi Radix-dialog-olmayan yüzeylerde kullanılır */
